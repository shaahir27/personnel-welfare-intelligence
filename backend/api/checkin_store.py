...
----------------------------------
Answers are never fed back into scoring. The eight behavioral signals are
...
def _coerce_answer(raw: Any) -> Dict[str, Any]:
    ...
    Returns:
        ...
        (0-4 scale) or ``text`` (free text).

    Raises:
        InvalidSubmission: If the entry is not a dict, has no question id, or
            carries neither a usable scale value nor text.
...
def record_submission(
    ...
    record = {
        ...
        "answers": [_coerce_answer(entry) for entry in answers],
    }
