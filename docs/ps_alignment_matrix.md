...
| **Stress/Burnout Risk Prediction Model(s)** | `backend/models/` | ✅ Full — 8 candidates trained, Gradient Boosting selected (R²=0.821); deployed model carries split-conformal calibrated intervals (±9.9 at 90%, verified 91.5%) |
...
| **#3: Minimising false positives / false negatives** | **Calibrated intervals:** every score carries a split-conformal range (±9.9 points, 90% coverage guaranteed in finite samples, verified 91.5% on unseen people); a case whose range crosses a cutoff is marked *borderline* on both screens and in the officer alert, so a provisional High is never presented as a settled one. Wide Moderate band (40–65) intentionally reduces false negatives. Officer visibility gated by one rule — High, or persistent *and rising* Moderate — which cut the queue from 619 to 159 of 800. Low-confidence alerts suppressed for officer/commander. Handling note appended to every case detail response. |
